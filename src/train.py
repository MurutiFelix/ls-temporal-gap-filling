...
def main():
    ...
    print("=" * 60)
    print("  RBFN Gap-Fill Pipeline Training")
    print("=" * 60)
    ...
    # Fetch reference spatial metadata from DEM
    ...
    for year in range(training_start, training_end + 1):
        for month in range(1, 13):
            ...
            X_month = np.hstack([
                dem_cube.reshape(h * w, -1),
                coords_2d,
                time_grid,
                era5_cube.reshape(h * w, -1)
            ])
            Y_month = landsat_cube.reshape(h * w, -1)
    ...
    # Correct sequence: Normalize Y targets
    Y_raw = processor.normalize_reflectance(Y_raw)

    # Temporal / Holdout Split FIRST to prevent data leakage
    ...
    print(f"  ✓ Model & Scaler successfully saved to {checkpoint_dir}")
